...
# <--- ФІНАЛЬНА ЗМІНА: ПОВЕРТАЄМОСЯ ДО НАЙБІЛЬШ СТАБІЛЬНОЇ БАЗОВОЇ МОДЕЛІ
...
async def generate_content(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    try:
        # Покращений промпт для кращої структури відповіді
        prompt = (
            f"Створи 3 унікальні ідеї для контенту в соціальній мережі {platform} на тему '{topic}'. "
            f"Для кожної ідеї надай: привабливий заголовок, короткий опис/текст для посту та добірку з 5-7 релевантних хештегів. "
